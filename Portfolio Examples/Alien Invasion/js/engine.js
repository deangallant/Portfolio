...
var Engine = (function(global) {
    ...
    function checkCollisions() {
        for (let i = 0; i < allEnemies.length; i++) {
          ...
          if (distance < player.size + allEnemies[i].size) {

            livesNumber.innerHTML = parseInt(livesNumber.innerHTML) - 1;
            if (parseInt(livesNumber.innerHTML) < 1) {
        ...
        const sx = player.x - sun.x;
        const sy = player.y - sun.y;
        const distance = Math.sqrt(sx * sx + sy * sy);
        if (distance < player.size + sun.size) {
          levelNumber.innerHTML = parseInt(levelNumber.innerHTML) + 1;
          allEnemies.forEach(elem => elem.velX = elem.velX + 1);
          player.x = 450;
          player.y = 400;
        }
    }
